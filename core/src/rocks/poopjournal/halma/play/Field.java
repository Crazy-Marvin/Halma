...
public class Field extends Image {
    ...
    public void clicked(){
        if(besetzt()) {
            ...
            try{showAllWays();}
                ...
                System.out.println("ojeee overfoww..");
            }
        }
    ...
    public void showAllWays() throws StackOverflowError{
        ...
        for(Field neighbour : neighbours) {
            ...
            if(neighbour.besetzt()) //oder kann ich springen?
                try{jump(neighbour);} catch (StackOverflowError e){System.out.println("overflow in all ways"); return;}
        }
        System.out.println(jumpCount);
        jumpCount = 0;
    }
    public void jump(Field neighbour) throws StackOverflowError{
        if(jumpCount>10_000) return;
        jumpCount++;
        ...
        if(!toJump.besetzt()) { //einmal checken, ob das feld frei ist
            ...
            try{toJump.afterJump(neighbour);} catch (StackOverflowError e){System.out.println("overflow in jump"); return;}//kann ich noch weiterspringen?
        }
    }
    public void afterJump(Field fromField) throws StackOverflowError{
        if(jumpCount>10_000) return;
            ...
            if(field != null) //ist das überhaupt ein nachbar?
                if(field.besetzt()) //kann ich dort drüber springen?
                    if(field != fromField) //Springe ich eh nicht zurück?
                        try{jump(field);} catch (StackOverflowError e){System.out.println("overflow in after jump");return;} //dann springen
    }
